...
impl Connector for PlainConnector {
    ...
    fn connect(url: &Url<'_>, _tls_config: &()) -> Result<Self::Stream, Error> {
        ...
        Ok(PlainStream(TcpStream::connect(format!(
            "{}:{}",
            host,
            url.effective_port()
        ))?))
...
mod small {
    ...
    use super::{
        Method, io_uring_pool, network_enabled, port_small_ureq, port_small_uring,
        port_small_xibalba, ureq_agent, xibalba_client,
    };
...
mod large_resp {
    ...
    use super::{
        Method, io_uring_pool, network_enabled, port_large_resp_ureq, port_large_resp_uring,
        port_large_resp_xibalba, ureq_agent, xibalba_client,
    };
...
mod large_req {
    ...
    use super::{
        LARGE_QUERY, Method, io_uring_pool, network_enabled, port_large_req_ureq,
        port_large_req_uring, port_large_req_xibalba, ureq_agent, xibalba_client,
    };
...
mod stress {
    ...
    use super::{
        Method, io_uring_pool, network_enabled, port_stress_ureq, port_stress_uring,
        port_stress_xibalba, ureq_agent, xibalba_client,
    };
...
mod concurrent {
    ...
    use super::{
        CONNS, REQS_PER_CONN, network_enabled, port_concurrent_ureq, port_concurrent_uring,
        ureq_agent,
    };
