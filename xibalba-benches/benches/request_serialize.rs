//! Serialization benches for xibalba's own `Request`.
//!
//! A comparison against the `http` crate's request builder lives under
//! `examples/` — see `examples/compare_request_serialize.rs` — so a
//! competing crate never shows up in this package's normal dependency graph.
