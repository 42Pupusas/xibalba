...
///
/// For ASCII alpha bytes, OR-ing with 0x20 normalizes to lowercase.
/// The range check guards against non-alpha bytes aliasing.
#[inline]
...
impl<'a> HeaderName<'a> {
    ...
    ///
    /// For `Unknown` variants this uses `from_utf8`. RFC 7230 header names
    /// are always ASCII tokens, so this is safe for any valid header name.
    /// Returns `"<invalid>"` for malformed bytes (should not happen with
    /// validated input).
    #[must_use]
    ...
    /// Returns a well-known variant if recognized, otherwise `Unknown`.
    /// Dispatches on length first to minimize comparisons.
    #[must_use]
...
impl Header<'_> {
    ...
    /// The sentinel value is always overwritten before being read.
    #[must_use]
...
mod tests {
    ...
    fn ascii_eq_non_alpha_no_alias() {
        // b'@' (0x40) | 0x20 = b'`' (0x60), but these should NOT match
