...
impl<'a> Url<'a> {
    ...
    /// # Errors
    ///
    /// Returns `Error::Url` variants for malformed input: empty input,
    /// missing/invalid scheme, missing host, or invalid port.
    ...
    /// Splits on `&` then on the first `=` in each pair.
    /// Returns an empty iterator if there is no query string.
    #[must_use]
...
/// Parse the authority component: host and optional port.
...
fn parse_authority(authority: &[u8]) -> Result<(&[u8], Option<u16>), Error> {
    ...
    if authority[0] == b'[' {
        ...
        // Regular host — find last ':' for port separator.
...
/// Parse port from ASCII digits without going through str.
fn parse_port(bytes: &[u8]) -> Result<u16, Error> {
    ...
    // Safe: we checked result <= u16::MAX above.
    #[allow(clippy::cast_possible_truncation)]
    Ok(result as u16)
}

/// Split the portion after the authority into path, query, and fragment.
fn parse_path_query_fragment(input: &[u8]) -> (&[u8], Option<&[u8]>, Option<&[u8]>) {
    if input.is_empty() {
        return (b"", None, None);
    }

    // Split at '#' first (fragment is never sent to server, but we parse it).
